...
const SignUp: React.FC = () => {
  ...
  const [formData, setFormData] = useState({ nome: "", email: "", senha: "", confSenha: "" });
  ...
  const handleSubmit = async (event: SyntheticEvent<HTMLFormElement, SubmitEvent>) => {
    ...
    try {
      ...
      if (event.nativeEvent.submitter?.id === "btnForm") {
        ...
        loginService.cadastroRenderizador(formData.nome, formData.email, formData.senha).then((response) => {

          if (response.status === 201) {

            //Faz Login
            loginService.doLoginRenderizador(formData.email, formData.senha).then((response2) => {

              if (response2.status === 200) {
    
                localStorage.setItem("auth-token",   response2.data.session);
                localStorage.setItem("renderizador", JSON.stringify(response2.data.renderizador));
  ...
  return (
    <OutLayout>
      ...
      <div className="login-out rounded-sm bg-white">
        <div className="w-full p-20 sm:p-06 xl:p-10 ">

          <h2 className="mb-9 text-2xl font-bold text-black dark:text-white sm:text-title-xl2">
            Faça Parte da Renderizaí
          </h2>
      ...
      { /*  ALERT  */}
      <div className="absolute top-20 right-10"> { /* Toast Container */ }
        
        <div className="hidden toast flex w-full border-l-6 border-[#F87171] bg-[#F87171] bg-opacity-[15%] px-7 py-8 shadow-md dark:bg-[#1B1B24] dark:bg-opacity-30 md:p-9">
        <button className="hover m-4" onClick={closeAlert}>

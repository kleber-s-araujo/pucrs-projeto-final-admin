      ...
      try {
        ...
        dashboardService.getTotalizadores(id).then((response) => {
          ...
          if (response.status === 200) {
            ...
            const saldoAteMesAnterior = data.totalRenders - data.qtdMesAtual;
            const taxaDeAumentoQtdTotal = ((data.qtdMesAtual / saldoAteMesAnterior) * 100).toFixed(2);
            const taxaDeAumentoMes = (((data.qtdMesAtual - data.qtdMesAnterior) / data.qtdMesAnterior) * 100).toFixed(2);
            const taxaAumentoSaldoMes = (((data.valorMesAtual - data.valorMesAnterior) / data.valorMesAnterior) * 100).toFixed(2);
            const taxaAumentoSaldoTotal = ((data.valorMesAtual / data.valorTotal) * 100).toFixed(2);

            //Formatar Moeda
            data.valorTotal = getFormattedCurrency(parseFloat(data.valorTotal));
            data.valorMesAtual = getFormattedCurrency(parseFloat(data.valorMesAtual));
